...
void M68k::ExecuteOpcode(word opcode)
{
	...
	{//1011
		...
		if(get().IsOpcode(opcode, "1011xxxx11xxxxxx"))
		{
			if(get().unitTests)
			{
				std::cout << "\tM68k :: Execute OpcodeCMPA" << std::endl;
			}

			get().OpcodeCMP_CMPA(opcode);
		}
		else if(get().IsOpcode(opcode, "1011xxx1xx001xxx"))
		{
			if(get().unitTests)
			{
		...
		else
		{
			if(get().unitTests)
			{
				std::cout << "\tM68k :: Execute OpcodeCMP" << std::endl;
			}
	...
	else if(get().IsOpcode(opcode, "0100100x1x000xxx"))
	{
		if(get().unitTests)
		{
...
void M68k::OpcodeASL_ASR_Register(word opcode)
{
	...
	get().opcodeClicks += (6 + (2 * shiftCount)); 
...
void M68k::OpcodeBCLRStatic(word opcode)
{
	...
	get().opcodeClicks += (size == BYTE) ? 8 : 10;
	get().opcodeClicks += dest.cycles;
}
...
void M68k::OpcodeBTSTDynamic(word opcode)
{
	...
	get().opcodeClicks += (size == BYTE) ? 4 : 6;
	get().opcodeClicks += dest.cycles;
}
...
void M68k::OpcodeCHK(word opcode)
{
	...
	get().opcodeClicks += 10;
	get().opcodeClicks += bounds.cycles;
...
void M68k::OpcodeCMPI(word opcode)
{
	...
	switch(type)
	{
		case EA_DATA_REG:
		case EA_ADDRESS_REG:
		get().opcodeClicks += (size == LONG) ? 14 : 8;
		break;
